"""Transfer learning script for diffusion model on new seismic data from different location.

This script loads a pre-trained DiT/EDM diffusion model and fine-tunes it on seismic data
...
def run(args):
    ...
    if args.learning_rate is not None:
        config.diffusion_learning_rate = args.learning_rate
    if args.max_steps is not None:
        config.diffusion_max_steps = args.max_steps
    ...
    is_dit = "DiT" in str(source_checkpoint_path) or args.model_type == "dit"
    is_edm = "EDM" in str(source_checkpoint_path) or args.model_type == "edm"

    if not is_dit and not is_edm:
        raise ValueError(
            "Could not determine model type from checkpoint path. "
            "Use --model-type to specify 'dit' or 'edm'"
        )
    ...
    if args.name is not None:
        ...
        model_name = "DiT" if is_dit else "EDM"
        name = f"Transfer-{model_name}-{spectr.shape[1] // 4}x{spectr.shape[2] // 4}x{config.latent_channels}-LogSpectrogram"
    ...
    optimizer_params = {
        ...
        "max_steps": config.diffusion_max_steps,
        "warmup_steps": config.diffusion_warmup_steps,
        "decay_steps": config.diffusion_max_steps,
        "end_learning_rate": config.diffusion_end_learning_rate,
    ...
    if args.transferred_autoencoder_checkpoint is not None:
        ...
        autoencoder_checkpoint = (
            config.outputdir
            / f"Transfer-Autoencoder-{spectr.shape[1] // 4}x{spectr.shape[2] // 4}x4-LogSpectrogram"
            / "last.ckpt"
        )
    ...
    if args.freeze_autoencoder:
        ...
        logging.info("Autoencoder will be fine-tuned jointly with diffusion model")

    # Load pre-trained diffusion model from source domain
    ...
    if is_dit:
        logging.info("Model type: DiT (Diffusion Transformer)")
        model = LightningDiT.load_from_checkpoint(
        ...
        logging.info("Model type: EDM (Elucidating Diffusion Models)")
        model = LightningEDM.load_from_checkpoint(
            source_checkpoint_path,
            autoencoder=autoencoder,
            optimizer_params=optimizer_params,  # Update optimizer for transfer learning
            num_sampling_steps=25,
...
if __name__ == "__main__":
    ...
    parser = argparse.ArgumentParser(
        description="Transfer learning for diffusion model on new seismic data"
    )
    parser.add_argument(
        "--workdir",
        type=str,
    ...
    parser.add_argument(
        ...
        type=str,
        required=True,
        help="Path to source diffusion checkpoint (e.g., workdir/outputs/Latent-DiT-.../last.ckpt)",
    )
    parser.add_argument(
    ...
    parser.add_argument(
        ...
        default=None,
        choices=["dit", "edm"],
        help="Model type (dit or edm). Auto-detected from checkpoint path if not specified.",
    )
    parser.add_argument(
        "--freeze-autoencoder",
        action="store_true",
