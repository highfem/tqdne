...
from tqdne.dataset import SpectrogramUpsamplingDataset
...
from tqdne.unet_2d import UNet2DModel
...
if __name__ == "__main__":
    ...
    name = "2D-UNET-UPSAMPLE-DDPM-noisy"
    config = Config()

    path_train = config.datasetdir / Path(config.data_upsample_train)
    path_test = config.datasetdir / Path(config.data_upsample_test)
    train_dataset = SpectrogramUpsamplingDataset(path_train, cut=t)
    test_dataset = SpectrogramUpsamplingDataset(path_test, cut=t)
    ...
    metrics = [
        SpectrogramInversion(
            m, stft_channels=config.stft_channels, hop_size=config.stft_hop_size
        )
        for m in metrics
    ]
