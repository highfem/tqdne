...
def check_linear_trend(waveform, r_squared_threshold=0.95, segment_threshold=0.10, min_segment_length=300):
    """
    ...
    if r_squared_full > r_squared_threshold:
        return True, (slope_full, intercept_full, r_squared_full, {'start': 0, 'end': n_samples-1, 'length': n_samples})
    ...
    for window_size in sorted(window_sizes, reverse=True):
        ...
        slope_win, intercept_win, r_value_win, p_value, std_err = stats.linregress(x_window, y_window)
        ...
        if r_squared_win > r_squared_threshold and window_size / n_samples > segment_threshold:
            ...
            segment_info = {'start': deviation_point, 'end': end-1, 'length': end - deviation_point}
            return True, (slope_win, intercept_win, r_squared_win, segment_info)
...
def plot_waveform_analysis(waveform, results, title="Waveform Analysis"):
    ...
    if "last_oscillating_sample" in results and results["last_oscillating_sample"] is not None:
        ...
        plt.axvline(x=last_osc, color='g', linestyle='--', 
                  label=f'Last Oscillating Sample ({last_osc})')
    ...
    if results["has_trailing_zeros"] and results["trailing_zero_index"] is not None:
        ...
        plt.axvline(x=trailing_idx, color='orange', linestyle='--', 
    ...
    if results["is_linear_trend"]:
        ...
        if segment_info:
            ...
            plt.plot(x_segment, trend_line, 'r--', 
                    label=f'Linear Segment (R² = {r_squared:.4f})', linewidth=2)
            
            # Mark start point
            plt.axvline(x=start, color='purple', linestyle='--', 
                      label=f'Linear Trend Start ({start})')
            ...
            plt.plot(x, trend_line, 'r--', 
                    label=f'Linear Trend (R² = {r_squared:.4f})', linewidth=2)
...
def evaluate_waveform_channel(waveform, channel_name=None, plot=False, r_squared_threshold=0.95, 
                            segment_threshold=0.10, min_segment_length=300, range_threshold=1e-5):
    """
    ...
    results["is_faulty"] = results["has_trailing_zeros"] or results["is_linear_trend"] or results["has_small_range"]
...
def evaluate_multi_channel_waveform(waveform_data, plot=False, r_squared_threshold=0.95, 
                                  segment_threshold=0.10, min_segment_length=300, range_threshold=1e-5):
    """
...
def print_largest_last_oscillating_sample(waveform_data, plot=False):
    ...
    for i, result in enumerate(channel_results):
        ...
        if is_flat and is_nonzero:
            ...
            if plot:
                plot_waveform_analysis(waveform_data[:, i], result, title=f"Channel {i+1} - Adjusted to Trailing Zeros")
    ...
    if save_to_file:
        try:
            with open(save_to_file, 'w') as f:
                # Write header
                f.write("waveform_number,largest_last_oscillating_sample,is_faulty,has_trailing_zeros,has_linear_trend,has_small_range\n")
                
                # Write data for each waveform
                for result in results:
                    f.write(f"{result['waveform_number']},{result['largest_last_oscillating_sample']},"
