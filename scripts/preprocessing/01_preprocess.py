...
for ieq, fname in enumerate(fList, start=1):
    ...
    useme = (np.array(recs['rhyp']) >= p['rhmin']) & (np.array(recs['rhyp']) <= p['rhmax']) & \
            (zz >= p['zmin']) & (zz <= p['zmax'])
    ...
    if isLaptop:
        recs['z_fullnames'] = [x.replace('/scratch/kpalgunadi', '/Users/mameier')
                               for x in recs['z_fullnames']]
        recs['e_fullnames'] = [x.replace('/scratch/kpalgunadi', '/Users/mameier')
                               for x in recs['e_fullnames']]
        recs['n_fullnames'] = [x.replace('/scratch/kpalgunadi', '/Users/mameier')
                               for x in recs['n_fullnames']]
