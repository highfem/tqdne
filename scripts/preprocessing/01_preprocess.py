...
def read_earthquake_data_full(hdf5_path, file_index=0):
    ...
    with h5py.File(hdf5_path, 'r') as h5f:
        ...
        if file_index >= len(file_groups):
            raise IndexError(f"File index {file_index} not found. Available: 0-{len(file_groups)-1}")
...
for ieq, (mag, file_index, group_name, filename) in enumerate(selected, start=1):
    ...
    useme = (rhyp_array >= p['rhmin']) & (rhyp_array <= p['rhmax']) & \
            (zz >= p['zmin']) & (zz <= p['zmax'])
...
if processed_earthquakes:
    ...
    print(f"Total records across all earthquakes: {sum(eq['_records_kept'] for eq in processed_earthquakes)}")
    print("\n" + "x" * 80)
else:
