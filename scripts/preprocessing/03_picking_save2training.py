...
for num_event in range(len(uniq_id)):
    ...
    for idx in data_filter['index']:
        ...
        if (np.isnan(np.mean(data_array[0, :])) == 1) or (np.isnan(np.mean(data_array[1, :])) == 1) or ((np.isnan(np.mean(data_array[2, :])) == 1)):
            print("there is nan")
            continue
        ...
        is_onshore = classify_hypocenter([float(metadata.source_latitude_deg[idx])], [float(metadata.source_longitude_deg[idx])], japan_polygon)
    ...
    for u in range(0, len(st), 3):
        ...
        if pn_picks:
            ...
            default_trace_id = (
                f"{stream_3c[0].stats.network}."
                f"{stream_3c[0].stats.station}."
            )
        ...
        if ave_start > 1.5e-4:
            iter += 1
            print(f"skip waveforms, Max amplitude > 1.5e-4")
            st_extract[0].data = np.zeros((len(st_extract[0].data)))
            st_extract[1].data = np.zeros((len(st_extract[1].data)))
            st_extract[2].data = np.zeros((len(st_extract[2].data)))
            st_data = np.vstack([tr.data for tr in st_extract]).T
