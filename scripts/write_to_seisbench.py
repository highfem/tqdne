...
parser = argparse.ArgumentParser(
    description="Convert the generative waveform model to seisbench format for a particular event"
)
parser.add_argument(
    "path_metadata",
    help="path to station metadata (*.csv), Assumes you have a stations.csv file with columns 'network', 'station_code', 'channel_name' 'latitude', 'longitude', 'hypocentral_distance'",
)
parser.add_argument("path_gwm", help="path to generative waveform model (*.h5)")
parser.add_argument(
    "--origin_time",
    nargs=1,
    default=(["2024-02-01T00:00:00.0"]),
    help="origin time when the earthquake occurred",
)
parser.add_argument(
    "--hypocenter",
    nargs=3,
    default=([0, 0, 10]),
    help="hypocenter coordinate for latitude (degree), longitude (degree), depth (km)",
)
...
parser.add_argument(
    ...
    nargs=1,
    default=([100.0]),
    help="sampling rate of the training data default is 100 samples/second",
)
...
stations_df = pd.read_csv(
    args.path_metadata
)  # Assumes you have a stations.csv file with columns 'network', 'station_code', 'latitude', 'longitude'
...
unique_combinations, indices, counts = np.unique(
    structured_array, return_index=True, return_counts=True
)
indices = np.sort(indices)
stations_df_filter = stations_df[indices, :]
stations_df = pd.DataFrame(
    stations_df_filter, columns=["Network", "Station", "Latitude", "Longitude", "Distance"]
)
    ...
    for i in range(n):
        ...
        for j in range(waveforms_per_station):
            ...
            stream = create_obspy_stream(
                waveform, channels, source_origin_time, trace_sampling_rate, station
            )
            ...
            trace_start_time = (
                source_origin_time + travel_time - trace_start_sample / trace_sampling_rate
            )
