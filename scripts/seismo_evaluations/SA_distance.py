...
dataset_path = glob.glob('../experiments/workdir/new_highfem1/h5_generativeGMM/for_GenerativeGMM*.h5')
...
# for ii in range(1):
    ...
    files = SeismicParameters("../experiments/workdir/new_highfem1/data/preprocessed_waveforms.h5")
    rhyp_obs = files.hypocentral_distance
    ...
    for j in range(4):
        ...
        for im in range(len(rhyp_filt2)):
            ax.scatter(rhyp_filt2[im], sa_mean2[im], marker='s', s=40, color='grey', alpha=0.9, label='Data-bin med.')
            ax.plot([rhyp_filt2[im], rhyp_filt2[im]], [sa_mean2[im], sa_162[im]], '-', color='grey', alpha=0.9)
            ax.plot([rhyp_filt2[im], rhyp_filt2[im]], [sa_mean2[im], sa_842[im]], '-', color='grey', alpha=0.9)
        for imt in range(len(imts)):
                for gsi, color in zip(range(len(gmpes)), colors):
                    ax.plot(jb_distances[idx], np.exp(gms[0, gsi, imt][idx])*9.81, linewidth=2, color=color, label=gmpes[gsi])
                    ax.plot(jb_distances[idx], np.exp(gms[0, gsi, imt][idx] + gms[1, gsi, imt][idx])*9.81, '--', linewidth=1, color=color, alpha=0.9)
                    ax.plot(jb_distances[idx], np.exp(gms[0, gsi, imt][idx] - gms[1, gsi, imt][idx])*9.81, '--', linewidth=1, color=color, alpha=0.9)
        if sa_mean_obs.shape[0] > 0:
            ax.scatter(rhyp_obs, sa_mean_obs[:,period], color='k', s=4, linewidth=0.2, edgecolor='k', label='Data')
            ax.set_title(f"V$_{{S30}}$: {vs30 - 50}-{vs30 + 50} m/s,  M{mags-0.1}-{mags+0.1}, T = {periods[period]}s, N$_{{obs}}$: {sa_mean_obs[:,period].shape[0]}", fontsize=14)
        else:
            ax.set_title(f"V$_{{S30}}$: {vs30 - 50}-{vs30 + 50} m/s, M{mags-0.1}-{mags+0.1}, T = {periods[period]}s, N$_{{obs}}$: 0", fontsize=14)
        ...
        ax.set_xlim(1,190)
        ax.set_ylim(1e-5, 20)
        fig.savefig(f"figures/updated/SA/SA_distance_Vs30{vs30}_M{mags}_T{periods[period]}s_log2.svg", dpi=200, bbox_inches="tight")
