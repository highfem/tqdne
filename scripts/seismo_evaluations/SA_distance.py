    ...
    for j in range(4):
        ...
        for imt in range(len(imts)):
                for gsi, color in zip(range(len(gmpes)), colors):
                    ax.plot(jb_distances[idx], np.exp(gms[0, gsi, imt][idx]), linewidth=2, color=color, label=gmpes[gsi])
                    ax.plot(jb_distances[idx], np.exp(gms[0, gsi, imt][idx] + gms[1, gsi, imt][idx]), '--', linewidth=1, color=color, alpha=0.9)
                    ax.plot(jb_distances[idx], np.exp(gms[0, gsi, imt][idx] - gms[1, gsi, imt][idx]), '--', linewidth=1, color=color, alpha=0.9)
        ...
        ax.set_xlim(1,190)
        ax.set_ylim(1e-6, 10)
        fig.savefig(f"figures/SA/nolog_new/SA_distance_Vs30{vs30}_M{mags}_T{periods[period]}s_nolog.png", dpi=200, bbox_inches="tight")
